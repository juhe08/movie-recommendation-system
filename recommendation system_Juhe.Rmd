...
We then observe the effects of users, movies, dates and genres. In real life, some movies tend to have higher marks than average, while some movies tend to get more low rating scores. This phenomenon leads to a movie bias between different movies, thus we add a new term $b_i$ to Equation (1) to represent the specific effect of each movie $u=i$. The value of $b_i$ is the mean value of the difference between true ratings of movie $i$ and $\mu$:
...
Due to the same reasons, we add a term $b_u$ for each user $u$ and a term $g_{u,i}$ for each genre and genre combination. 
...
train_set %>% mutate(date = as_datetime(timestamp)) %>% mutate(date = round_date(date, unit = "month")) %>%
       ...
       geom_point() +
       geom_smooth() + labs(title = "Average rating change with") + theme(plot.title = element_text(hjust = 0))
```
